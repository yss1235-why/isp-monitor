...
// Original ISP Monitor with silent UptimeRobot integration
...
// Silent UptimeRobot integration (loads data in background, no UI changes if no data)
class UptimeRobotIntegration {
    ...
    async loadUptimeRobotData() {
        try {
            ...
            if (response.ok) {
                ...
                this.hasData = data.monitors && data.monitors.length > 0;
                return data.monitors || [];
            }
        } catch (error) {
            // Silent fail - no console logs, no UI changes
        }
        return [];
    }
    ...
    async updateExternalStatus() {
        ...
        // Only update status if we have data, otherwise keep "Setting up..."
        if (this.hasData && monitors.length > 0) {
            ...
            if (statusElement) {
                if (upCount === totalCount) {
                    ...
                    statusElement.innerHTML = `⚠️ ${upCount}/${totalCount} UP`;
                    statusElement.style.background = '#fff3cd';
                    statusElement.style.color = '#856404';
                }
            }
        }
        // If no data, leave status as "Setting up..." (unchanged)
    }
}

// Main ISP Monitor Class (keeps original functionality)
class ISPMonitor {
    constructor() {
        this.data = [];
    ...
    async init() {
        ...
        // Silent UptimeRobot status update (no new sections)
        await this.uptimeRobot.updateExternalStatus();
        
        // Auto-refresh every 5 minutes
        setInterval(() => {
            this.loadData().then(() => {
                this.updateDashboard();
                this.uptimeRobot.updateExternalStatus();
            });
        }, 5 * 60 * 1000);
    }
